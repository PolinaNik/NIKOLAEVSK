...
class RoundRoute:
    ...
    def first_turn(self):
        ...
        if turn1 == 'LEFT':
            arc = create_arc(change_angle(start_bearing - 170), change_angle(start_bearing + 90), round_step,
                             glob_center,
                             glob_rad)
            arc.reverse()
            ...
            arc = create_arc(change_angle(start_bearing - 90), change_angle(start_bearing + 170), round_step,
                             glob_center,
                             glob_rad)
            arc_points = [convert.to_gr(arc[num]) for num, item in enumerate(arc)]
            return arc_points
    ...
    def second_turn(self):
        ...
        if turn2 == 'LEFT':
            center = intersection_point(end_arc, last_cord1, change_angle(end_bearing + 90),
                                        change_angle(turn_bearing + 90))
