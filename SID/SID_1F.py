...
OSBET = ParalelRoute(start_point=start_point, start_bearing=288,
              begin_arc=begin_arc, turn1='LEFT', glob_center=gl_center_down, glob_rad=gl_rad_down, kink=kinks.ratnu,
...
draw_route(file_name, 'TOTRU1F', convert.to_gr(start_point), convert.to_gr(begin_arc), TOTRU.first_turn(), convert.to_gr(points.TOTRU))

draw_route(file_name, 'DOKIR1F', convert.to_gr(start_point), convert.to_gr(begin_arc), DOKIR.first_turn(), convert.to_gr(points.DOKIR))

draw_route(file_name, 'KIZON1F', convert.to_gr(start_point), convert.to_gr(begin_arc), KIZON.first_turn(), convert.to_gr(points.KIZON))

draw_route(file_name, 'NOKDA1F', convert.to_gr(start_point), convert.to_gr(begin_arc), NOKDA.first_turn(), convert.to_gr(points.NOKDA))

draw_route(file_name, 'GIGOR1F', convert.to_gr(start_point), convert.to_gr(begin_arc), GIGOR.first_turn(), convert.to_gr(points.GIGOR))

draw_route(file_name, 'OSBET1F', convert.to_gr(start_point), convert.to_gr(begin_arc), OSBET.first_turn()[:-2], convert.to_gr(points.OSBET))

draw_route(file_name, 'LIRSA1F', convert.to_gr(start_point), convert.to_gr(begin_arc), LIRSA.first_turn(), convert.to_gr(points.LIRSA))

draw_route(file_name, 'LEKBI1F', convert.to_gr(start_point), convert.to_gr(begin_arc), LEKBI.first_turn(), convert.to_gr(points.LEKBI))
